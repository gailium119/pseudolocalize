...
size_t base64_encode(const unsigned char in[], unsigned char out[], size_t len, int newline_flag)
{
	...
	if (out == NULL) {
		...
		if (left_over)
			idx2 += 4;
		if (newline_flag)
			idx2 += len / 57;   // (NEWLINE_INVL / 4) * 3 = 57. One newline per 57 input unsigned chars.
	}
	else {
		// Since 3 input unsigned chars = 4 output unsigned chars, determine out how many even sets of
		// 3 unsigned chars the input has.
		blk_ceiling = blks * 3;
